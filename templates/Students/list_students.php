      ...
      <?php
        ...
        ?>
              <tr>
                  ...
                  <td><?= $data->phone_no ?></td>
                  <td>
                        <form id="frm-delete-student-<?= $data->id ?>" action="<?= $this->Url->build('/delete-student/' . $data->id, ['fullBase' => false]) ?>" method="delete">
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" value="<?= $data->id ?>" name="id" />
                           
                        </form>
                        <a href="<?= $this->Url->build('/edit-student/' . $data->id, ['fullBase' => true]) ?>" class="btn btn-warning">Edit</a>
                        <a href="javascript:void(0)" onclick="if(confirm('Are you sure want to delete ?')){ document.getElementById('frm-delete-student-<?= $data->id ?>').submit() }" class="btn btn-danger">Delete</a>
                  </td>
              </tr>
      <?php
            }
